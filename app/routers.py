...
async def create_user(user_data: users.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    user_data_dict["password_hash"] = hash_password(user_data.password)  # Хешируем пароль
    del user_data_dict["password"]  # Удаляем открытый пароль
    return await UserDAO.create(db, user_data_dict)
...
@router.put("/users/{user_id}", response_model=users.UserResponse)
async def update_user(
        user_id: int,
        user_data: users.UserUpdate,
        db: AsyncSession = Depends(get_db),
        current_user: users.UserResponse = Depends(get_current_user)
):
    ...
    if update_data:
        query = update(UserDAO.model.__table__).where(UserDAO.model.id == user_id).values(**update_data)
...
async def generate_blog_post(post_data: blog_posts.BlogPostCreate, db: AsyncSession = Depends(get_db),
                             current_user: users.UserResponse = Depends(get_current_user)):
...
async def create_telegram_subscriber(subscriber_data: telegram_subscribers.TelegramSubscriberCreate,
                                     db: AsyncSession = Depends(get_db)):
...
async def create_subscriber_preference(pref_data: subscriber_preferences.SubscriberPreferenceCreate,
                                       db: AsyncSession = Depends(get_db)):
